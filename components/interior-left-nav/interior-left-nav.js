...
import toggleClass from '../../ressources/js/misc/svg-toggle-class';
...
class InteriorLeftNav extends mixin(createComponent, initComponentBySearch, handles) {
  ...
  hookListItemsEvents = () => {
    this.manage(
      on(this.element, 'click', evt => {
        ...
        const collapseIcon = eventMatches(evt, `.${this.options.classLeftNavIcon}`);
        ...
        if (collapseEl || collapsedBar || collapseIcon) {
          evt.preventDefault();

          if (collapseEl || collapsedBar)
            console.log("Menu collasping")
        }
      })
    );
  ...
  handleNestedListClick(listItem, evt) {
    ...
    if (typeof evt.target.dataset !== 'undefined' && !('InteriorLeftNavItemLink' in evt.target.dataset)) {
  ...
  static get options() {
    ...
    return {
      ...
      classLeftNavIcon: `left-nav-list__item-icon`,
      // Event
